...
const autoBalanceTeams = () => {
    // Create a copy of players with actual data
    ...
    const sortedPlayers = [...validPlayers].sort((a, b) => b.skill - a.skill);
    ...
    // Update the teams in the UI
    teamA.value.players = teamAPlayers;
    teamB.value.players = teamBPlayers;
    
    // Auto-select captains (highest skill in each team)
...
const saveMatch = async () => {
    ...
    try {
        ...
        if (response.data.success) {
            alert('Match saved successfully! You can update the winner later.');
            resetTeams();
            fetchMatchRecords();
        }
    } catch (error) {
                        ...
                        <div v-if="showUpdateWinnerModal" class="bg-white rounded-xl shadow-2xl p-6 max-w-md w-full" @click.stop>
                            ...
                            <div v-if="selectedMatch" class="space-y-4">
                                <div class="p-4 bg-gray-50 rounded-lg">
                                    ...
                                    <div class="text-sm text-gray-500">Date: {{ formatDate(selectedMatch.match_date) }}</div>
                                </div>
